...
use crate::domain::vrm_system_model::utils::config::{
    MEMORY_PER_NODE, SCHEDULE_SYNC_TIMEINTERVAL_S, SLURM_RMS_COMMIT_TIMEOUT_S, SLURM_RMS_DELETE_TIMEOUT_S,
};
use crate::domain::vrm_system_model::utils::id::{ResourceName, RmsId, ShadowScheduleId, SlottedScheduleId};
use crate::domain::vrm_system_model::utils::state_logging::{AnalyticLogger, BaseLog, VrmCommand};
use crate::{
...
impl SlurmRms {
    pub fn log_base_info(&self, command: VrmCommand, log_description: String, reservation_id: ReservationId, arrival_time_at_aci: i64) {
        if let Some(base_log) = BaseLog::new(
            ...
            log_description,
            reservation_id,
            self.get_reservation_store().clone(),
