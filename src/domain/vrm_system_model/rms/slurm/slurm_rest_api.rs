...
impl SlurmRestApi for SlurmRestApiClient {
    ...
    async fn commit(&self, payload: TaskSubmission) -> Result<u32> {
        ...
        let response_data: TaskSubmitResponse = res
            .json()
            .await
            .context("Failed to parse submission response")?;

        // Return the Job id for meta-scheduler
        response_data.job_id.ok_or_else(|| {
            anyhow::anyhow!("Slurm accepted the job but didn't return an ID: {:?}", response_data.error)
        })
    }
    ...
    async fn delete(&self, task_id: u32) -> Result<bool> {
        ...
        let response: SlurmDeleteResponse = res.json().await.context("Failed to parse delete response")?;
        println!("{:?}", response);

        if let Some(errors) = response.errors {
            if !errors.is_empty() {
                anyhow::bail!("Slurm cancellation error: {:?}", errors);
            }
        }
